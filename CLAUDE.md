...
- The pinned path relies on PyNvVideoCodec queuing its copy on the stream it is
  ...
  recording's frame size, and False means record with the host upload. A pinned
  encoder whose first encode fails turns the pinned path off for the process
  (`upload_stats()["pinned_disabled"]`). A pinned setup failure falls back to the
  host upload for that encoder, with a `WARNINGS.txt` line. After a
