...
from gui_app.recording_meta import (camera_backend, camera_sort_key,
                                    trigger_source)
...
def block_rate_check(rec_dir: Path, names, blocks, fps: int):
    ...
    for nm, b in zip(names, blocks):
        ...
        times.append(ft[1])
        checked.append(nm)
    # The advice is the live router's: the recording's backend's clauses
    # (a FLIR camera has no .pfs to check) and the source
    # session_metadata.json records, because with an external source this
    # check is the only test of the operator's rate.
    hints = {**block_rate_hints(camera_backend(rec_dir)),
             "source_hint": source_name(trigger_source(rec_dir))}
