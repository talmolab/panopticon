...
The log is flushed at shutdown, from the excepthook and before `session.log`
is copied. On the main thread only, the excepthook waits up to a second for
the writer. `flush()` finishes only the calling thread's unfinished line. The
writer writes the file before it forwards a line anywhere else. Lines still on
the queue at a native crash are lost; faulthandler writes its traceback
straight to the file. `rig_setup.apply_profile_to_manager()` puts the
profile's level in force, so the window, the probes and the capture workers all
log at it.
